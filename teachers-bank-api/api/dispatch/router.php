...
function updateDispatch($id) {
    ...
    $chk  = $conn->prepare("SELECT id, dispatch_date, status FROM dispatch WHERE id = ?");
    ...
    if ($newStatus === 'Delivered' && empty($body['delivered_date'])) {
    ...
    $sets = []; $params = []; $types = '';
    foreach (['delivered_date', 'pod_date', 'status'] as $field) {
        if (isset($body[$field])) {
            $sets[]   = "$field = ?";
            $params[] = $body[$field];
            $types   .= 's';
