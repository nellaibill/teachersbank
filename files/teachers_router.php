...
function validateTeacher(array $body, bool $isCreate = true): array {
    ...
    $subs = is_array($body['sub_code'] ?? null) ? $body['sub_code'] : array_map('trim', explode(',', $body['sub_code'] ?? ''));
    if (empty(array_filter($subs, fn($s) => array_key_exists($s, SUBJECTS)))) $errors[] = 'At least one subject is required';

    $stds = is_array($body['std'] ?? null) ? $body['std'] : array_map('trim', explode(',', $body['std'] ?? ''));
    if (empty(array_filter($stds, fn($s) => in_array($s, STANDARDS)))) $errors[] = 'At least one standard is required';

    $meds = is_array($body['medium'] ?? null) ? $body['medium'] : array_map('trim', explode(',', $body['medium'] ?? ''));
    if (empty(array_filter($meds, fn($m) => array_key_exists($m, MEDIUMS)))) $errors[] = 'At least one medium is required';

    if (empty(trim($body['school_name'] ?? ''))) $errors[] = 'School name is required';
    if (empty($body['school_type']) || !in_array($body['school_type'], SCHOOL_TYPES)) $errors[] = 'School type is required';
...
function getTeachers() {
    ...
    $stmt->bind_param($types . 'ii', ...[...$params, $limit, $offset]); $stmt->execute();
...
function createTeacher() {
    ...
    $remarks         = !empty($body['remarks']) ? sanitize($body['remarks']) : null; // ← NEW

    $stmt = $conn->prepare("
        INSERT INTO teachers
            (teacher_name, contact_number, teacher_address, pincode,
             dt_code, sub_code, std, medium, school_name, school_type, remarks, isActive)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
    ");
    $stmt->bind_param('sssssssssss',                                          // ← 11 strings
        $teacher_name, $contact_number, $teacher_address, $pincode,
        $dt_code, $sub_code, $std, $medium, $school_name, $school_type, $remarks
    );
    ...
    $upd->bind_param('si', $barcode, $teacherId); $upd->execute();

    $sel = $conn->prepare("SELECT * FROM teachers WHERE id = ?");
    $sel->bind_param('i', $teacherId); $sel->execute();
...
function getTeacher($id) {
    ...
    $dispatches = []; $res = $dStmt->get_result();
...
function updateTeacher($id) {
    ...
    $std             = normaliseCsv($body['std']      ?? [], STANDARDS);
    $medium          = normaliseCsv($body['medium']   ?? [], array_keys(MEDIUMS));
    $remarks         = array_key_exists('remarks', $body)                          // ← NEW
    ...
    $stmt = $conn->prepare("
        ...
        WHERE id=?
    ");
    $stmt->bind_param('sssssssssssii',                                             // ← 11s + 2i
        $teacher_name, $contact_number, $teacher_address, $pincode,
        $dt_code, $sub_code, $std, $medium,
